...
(async()=>{ const b=await launch({port:9363}); const J=s=>b.evalJS(s);
  try{ await b.goto('http://127.0.0.1:8090/sites/net/public/minicad/3d/?ff=1'); await b.waitFor('!!window.MC3DVIEW&&!!MC3DVIEW.FF'); await J(`localStorage.clear();MC3DVIEW.ffNew();'ok'`); await sleep(300);
    ...
    m=await J(`(()=>{ var s=null,ring=null; MC3DVIEW.scene.children.forEach(o=>{ if(o.isSprite&&o.visible&&o.userData.shape==='square') s=o; if(o.isSprite&&o.visible&&o.renderOrder===1001) ring=o; }); return s?{ok:true,col:s.material.color.getHexString(),sz:s.scale.x<0.05,ring:!!ring,tip:document.getElementById('snaptip').textContent}:{ok:false}; })()`);
    ck(m.ok&&m.col==='2fa84f'&&m.sz&&m.ring&&m.tip==='끝점','끝점 호버 → 접촉 기호(초록 사각·화면 고정 크기) + 헤일로 링 + 이름표 '+JSON.stringify(m));
    // 안내점: 줄자 Ctrl+클릭
