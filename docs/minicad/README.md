...
- 검증: `cdp-ff.cjs` E2E 24건 — 진짜 포인터로 사각형→면→매스→그립 경사, **채널 전송 0건 확인**,
  ...
  연동 뷰 회귀 전부 통과(왕복 22·그립 21·두 창 E2E). 캐시 `3d25`.
- **다음 단계**(자유도 로드맵): ② 3D 스케치 그래프(평면별 면 검출 — 아무 데나 그리기) ③ 법선
  밀기끌기+`cuts[]`(벽감·아치) ④ Follow Me(몰딩) ⑤ 그룹·컴포넌트.

- **아직 안 된 것**: 세로 면 편집(벽감·아치 — 프리폼 ②·③ 단계에서).
