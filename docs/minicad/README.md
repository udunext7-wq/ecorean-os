...
- **4차 (같은 날, 대표 지시 "객체의 면이 선택될 수 있고 그 면을 편집할 수 있도록")** — 스케치업 그룹 편집 그대로:
  · **더블클릭 = 그룹 안으로**(`ST.editMass`, 나머지는 흐리게 0.3). 안에서 클릭 = **면**(파란 판+테두리), 모서리 8px 안 = **모서리**(굵은 파란 선).
    면·모서리는 인덱스가 아니라 **로컬 좌표**(`lp/ln`, `a/b`)로 기억 → 각기둥↔다면체 변환·재조립 뒤에도 같은 자리를 다시 잡는다(`ffReselectFace`, 이동/밀기 뒤엔 좌표를 따라 옮긴다).
  · 선택한 면: **M 이동**(기본=법선, ←→↑=세계 축, 숫자=거리 — op `moveverts`) · P 밀기끌기 · F 오프셋 · **B 페인트 = 그 면만**(Ctrl 불필요) · **Del = 면 삭제 → 열린 껍질**(`m.open`, 각기둥 복귀 금지) · 우클릭 면 뒤집기 · 개체 정보 = 면적·법선·재질.
  · 선택한 모서리: M 이동(바닥 방향, ↑=높이 → 박공) · Del = 모서리와 붙은 면 함께 삭제.
  · Esc 1회 = 선택 해제, 2회 = 밖으로 · 다른 매스·빈 곳 클릭 = 밖으로. 그룹 안에서는 선택 도구로 통째 끌기 안 됨(스케치업과 같다).
  · sketch.js: `massFaceRing/massEdges/massMoveVerts/massDeleteFace/massDeleteEdge/massReverseFace`. 검증 `ff-sketchup-4.cjs` 18건. 캐시 `3d38`.
