...
| **v5.5** | **2026-04-27** | **§91~§103 자동 연동 시스템, 부록 E·F·G 추가** |
...
| 컨셉 | 바닥 | 벽 | 천장 | 타일 | 조명 |
|------|------|----|------|------|------|
| 심플 모던 | 강마루 화이트오크 | 화이트 도장 | 화이트 도장 | 600×600 그레이 | 매립 다운라이트 |
| 클래식 럭셔리 | 원목 마루(월넛) | 베이지 실크 도배 | 우물천장+몰딩 | 대리석 패턴 | 샹들리에+매립 |
| 빈티지 레트로 | 헤링본 마루 | 그린/머스타드 | 우드 빔(옵션) | 모자이크/서브웨이 | 펜던트+직부등 |
| 내추럴 우드 | 원목 마루 | 베이지+우드 포인트 | 아이보리 도장 | 베이지 톤 | 우드 펜던트 |
| 북유럽 | 화이트 강마루 | 화이트+그레이 포인트 | 화이트 | 화이트+블랙 그라우트 | 매립+펜던트 |
| 인더스트리얼 | 짙은 마루 | 노출 콘크리트+벽돌 | 노출 천장 | 시멘트 패턴 | 메탈 펜던트 |
| 재팬 젠 | 원목(오크) | 회색 도장 | 베이지 도장 | 무광 베이지 | 종이 펜던트 |
| 프렌치 | 헤링본(라이트) | 화이트+와인스코트 몰딩 | 우물+화이트 | 대리석 | 작은 샹들리에 |
| 한국 모던 | 강마루(월넛/그레이) | 도배+한지 포인트 | 도장 | 한국 도자기 패턴 | 매립 |
| 스마트 홈 | 강마루 | 화이트+컬러 강조 | 매립+LED 라인 | 600×600 모던 | 스마트 LED 전체 |

스마트 홈 추가 적용: IoT 센서/스마트 스위치 자동 포함
...
30평 기준에서 드레스룸 확장(3×2m), 다용도실(2×1.5m), 알파룸(2.5×2m) 추가.

### 40평+ 대형 평형

주방/다이닝 분리, 다용도실, 게스트룸 포함. 안방 드레스룸 독립형.
...
*총 103섹션 + 7부록 | 2026-04-27 by udunext7-wq*
