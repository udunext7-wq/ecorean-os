...
### D-060. MiniCAD 소스 SSoT = `sites/net/public/minicad/`
- **결정**: MiniCAD 의 유일한 소스는 ecorean.net 실서비스 소스 `sites/net/public/minicad/` 다. 루트 `minicad/` 분리본·Desktop `MiniCAD-v5.9-Galaxy` 동기화 사본은 폐기(`archived/minicad-legacy-split-20260819/` 보존)하고, 갤럭시탭은 `https://ecorean.net/minicad/` 로 사용한다. `apps/minicad/` 는 v7 재배치용 자리표시자이며 코드를 두지 않는다.
- **근거**: 동일 기능이 3곳(루트 분리본·Desktop 사본·실서비스)에 갈라져 있어 어느 쪽을 고쳐야 하는지 매번 확인해야 했고(김비서 누락 패턴), 태블릿 대응 작업이 실서비스 쪽에만 누적됨. 실서비스 = 배포 = 사용자가 쓰는 것.
- **결정 주체**: 대표 (2026-08-19)
- **상태**: CONFIRMED
...
| 2026-08-19 | D-060 추가 (MiniCAD 소스 SSoT = sites/net/public/minicad). | 대표 + Claude Code |
