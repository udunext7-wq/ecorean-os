...
export async function GET(request: NextRequest) {
  ...
  try {
    if (op === 'floors') return await floors(loc);
...
/** 층별개요 — 지번 전체를 받아 동(mgm_pk)별로 묶어준다 */
async function floors(loc: Loc) {
  const { items, totalCount, truncated } = await hubAll('getBrFlrOulnInfo', loc, FLOOR_MAX_PAGES);
  const byDong: Record<string, { mgm_pk: string; dong_nm: string | null; bld_nm: string | null; floors: FloorRow[] }> =
    {};
