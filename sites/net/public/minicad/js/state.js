...
const STATE={
  ...
  lightArray:{cols:2,rows:2,dx:900,dy:900,angle:0,shape:'grid'},
  ...
  // 2026-09-07 대표 지적: "R 은 미니캐드에서 공간이었는데 그것을 바꿔 버리면 문제가 생긴다.
  //  기존 공간과 다른 개념으로 단축키를 따로 두고 새로 만드는 방식으로 가야 한다."
  //  맞는 말이다 — 프로토콜 6 이 R·C·L 의 뜻을 조용히 바꿔 놓았다(도구 이름은 '사각공간'인데
  //  공간이 안 만들어졌다). 공간 도구는 원래대로 되돌리고, z 값을 가진 자유 입체는
  //  '매스' 라는 별도 도구(N)로 뗐다. 이 스위치를 켜면 옛 프로토콜 6 방식으로 돌아간다.
  sketchMode:false, // true = 선·사각·원이 면이 되어 Z 를 묻는다 (명령 sk on)
  // 2026-09-09 천정 도구 (대표 지시): 프리=직접 그림 · 빠른작업=바닥(공간) 클릭 한 번
  ceilMode:'quick', ceilDrop:300,
